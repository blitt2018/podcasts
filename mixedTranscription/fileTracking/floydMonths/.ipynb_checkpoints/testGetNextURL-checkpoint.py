...
with sqlEngine.connect() as conn:
    with conn.begin():
        ...
        #conn.execute(text("LOCK TABLES podcasts.floydLinks WRITE;"))
        df = pd.read_sql("select * from podcasts.floydLinksOld WHERE processed = 0 LIMIT 1", conn, index_col="index")
        ...
        updateSQL = conn.execute(text(f"UPDATE podcasts.floydLinksOld SET processed = 1 WHERE `index` = {iVal};"))
